...
const Projects = () => {
  ...
  if (isError) {
    return <div className="text-center text-red-500 py-20">Failed to load projects.</div>;
  }
                  ...
                  <section className="flex items-center gap-3 justify-between mt-auto">
                    ...
                    <div className="relative">
                      ...
                      <motion.div
                        ...
                        initial="hidden"
                        animate={openDropdownId === project._id ? "visible" : "hidden"}
                        variants={dropdownVariants}
                        style={{ originY: "bottom" }}
                      >
                        {project.client_code && (
                          <a href={project.client_code} target="_blank" rel="noopener noreferrer" className="text-gray-300 text-xs md:text-sm border-r border-gray-600 pr-3 hover:text-cyan-400 transition-colors">
                            Client
                          </a>
                        )}
                        {project.server_code && (
                          <a href={project.server_code} target="_blank" rel="noopener noreferrer" className="text-gray-300 text-xs md:text-sm hover:text-cyan-400 transition-colors pl-3">
                            Server
                          </a>
                        )}
                      </motion.div>
